...
(defn issues-with-extra [config]
  (let [issues (get-all-issues config)]
    (map (partial assoc-comments-and-events config) issues)))
...
(defn format-comment [{:keys [user-map jira-project git-base-url]} c]
...
(defn issue2row [{:keys [user-map maxcmt] :as config} issue]
  (let [filtered-events (remove #(= "subscribed" (:event %)) (:event-contents issue))
    ...
    (concat
      (vector
        ...
        (get-labels issue))
      (map (partial format-comment config) trimmed-comments)
      (repeat (- maxcmt (count trimmed-comments)) "")    ; pad out field count  
    )))

(defn format-issues-to-export [{:keys [maxcmt] :as config} issues]
...
(defn process
  ...
  (let [config (load-config config-id)]
    (let [issues (issues-with-extra-cached config)
          export (format-issues-to-export config issues)]
